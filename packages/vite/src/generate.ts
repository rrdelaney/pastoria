...
export class PastoriaExecutionContext {
  ...
  private cachedTypeChecker: ts.TypeChecker | undefined;
  private cachedFileSystem: ReturnType<Project['getFileSystem']> | undefined;
...
`;
  ...
  private getTypeChecker() {
    this.cachedTypeChecker ??= this.project.getTypeChecker().compilerObject;
    return this.cachedTypeChecker;
  }

  private getFileSystem() {
    this.cachedFileSystem ??= this.project.getFileSystem();
...
`;
    ...
    // Collect parameters per-query, then merge. This avoids redundant lookups
    // when writeStandaloneEntryPoint needs per-query variable lists.
    const perQueryParams = new Map<string, Map<string, ts.Type>>();
    const params = new Map<string, ts.Type>();
    for (const [, queryName] of queries) {
      const queryParams = collectQueryParameters(this.project, [queryName]);
      perQueryParams.set(queryName, queryParams);
            ...
            initializer: (writer) => {
              writer.block(() => {
                ...
                if (getPreloadProps != null) {
                  ...
                  this.writeStandaloneEntryPoint(
                    writer,
                    resourceName,
                    queries,
                    perQueryParams,
   ...
   * @param perQueryParams Pre-collected parameters per query name, to avoid redundant type lookups.
   */
  private writeStandaloneEntryPoint(
    writer: CodeBlockWriter,
    resourceName: string,
    queries: Map<string, string>,
    perQueryParams: Map<string, Map<string, ts.Type>>,
  ) {
      ...
      writer.write('return').block(() => {
        ...
        writer.write('queries:').block(() => {
          for (const [queryRef, queryName] of queries) {
            const queryVars = perQueryParams.get(queryName);
            const hasVariables = queryVars != null && queryVars.size > 0;
