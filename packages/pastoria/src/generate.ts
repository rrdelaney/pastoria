...
async function loadRouterTemplate(
  project: Project,
  filename: string,
  templatesDir: string,
) {
  ...
  const template = await project.getFileSystem().readFile(templatePath);
...
async function generateRouter(
  ...
  const routerTemplate = await loadRouterTemplate(
    project,
    'router.tsx',
    templatesDir,
  );
...
`);
  ...
  for (let [
    ...
    const filePath = path.relative(projectDir, sourceFile.getFilePath());
...
async function generateServer(
  ...
  const serverTemplate = await loadRouterTemplate(
    project,
    'server.ts',
    templatesDir,
  );
...
async function generateEntryPointFiles(
  ...
  const generatedDir = path.resolve(projectDir, '__generated__/router');
  try {
    const entries = project.getFileSystem().readDirSync(generatedDir);
    for (const entry of entries) {
      if (
        ...
        project.getFileSystem().deleteSync(entry.name);
        logWarn('Removed stale entrypoint', pc.cyan(path.basename(entry.name)));
      }
    }
  } catch {
...
export async function generatePastoriaArtifacts(
  ...
  await generateServer(project, metadata, templatesDir, projectDir);
  await generateEntryPointFiles(project, metadata, projectDir);
}
